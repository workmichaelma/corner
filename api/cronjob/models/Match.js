...
MatchSchema.statics.find_Update_Insert = async _match => {
  try {
    const match = reduce(_match, (obj, v, k) => {
      if (v) {
        obj[k] = v
      }
      return obj
    ...
    return new Promise((resolve, reject) => {
      ...
      obj.validate(function (err) {
        if (err) {
          console.error('MatchSchema.statics.find_Update_Insert() error:', { doc: obj._doc, err })
          resolve({})
        }
        const doc = {
          ...obj._doc,
        ...
        delete doc._id
        if (doc.id || doc.winId) {
          resolve(Match.findOneAndUpdate({
            ...match.id ? {
              id: match.id
            } : {
              winId: match.winId
      ...
      console.error('MatchSchema.statics.find_Update_Insert() error: ', { _match, err })
    })
  } catch (err) {
    console.error('MatchSchema.statics.find_Update_Insert() error: ', { _match, err })
    return {}
...
MatchSchema.statics.getMatchesWithDateRange = async ({start: _start, end: _end = moment(), odds = false, result, limit = 100}) => {
  try {
...
MatchSchema.statics.getNoResultMatchesWithDateRange = async ({start: _start, end: _end = moment(), limit = 100}) => {
  try {
    const start = moment(_start)
    const end = moment(_end)
    ...
    if (start.isValid() && end.isValid()) {
      ...
      }).limit(limit).sort({datetime: 1}).lean()
    }
  } catch (err) {
    console.error(err)
    return []
...
MatchSchema.statics.saveMatchWithWin = async ({ jc, win, result }) => {
  try {
    ...
    const { id: winHomeId, rank: homeRank = '', homeImage = '' } = winHomeTeam || {}
    const { id: winAwayId, rank: awayRank = '', awayImage = '' } = winAwayTeam || {}
